 ...
 * @author Rushikesh
 * 
 * This is the main class used for invoking the Crawler instance and providing necessary configuration
 *
 */
public class Application {
	...
	public static boolean externalURLFlag = false;
	...
	public Application(String args[]) {
		...
		if (cmd.getOptionValue("externalURL") != null) {
			this.externalURLFlag = true;
		}
	...
	private static CommandLine configureCLI(String[] args) {
		...
		options.addOption(Option.builder("u").longOpt("url").required(true).hasArg(true)
				.desc("website to crawl (http://example.com)").build());

		options.addOption(Option.builder("o").longOpt("output-file").hasArg(true)
				.desc("output file - by default: sitemap.xml").build());

		options.addOption(Option.builder("c").longOpt("crawlers").hasArg(true)
				.desc("number of crawlers - by default: 1").build());

		options.addOption(Option.builder("m").longOpt("max-pages").hasArg(true)
				.desc("max pages to fetch - by default: 500").build());

		options.addOption(Option.builder("t").longOpt("temp-dir").hasArg(true)
				.desc("temporary directory - by default: tmp/").build());
		
		options.addOption(Option.builder("e").longOpt("externalURL").hasArg(true)
				.desc("External URL List visibility : by default false").build());
		options.addOption(
				Option.builder("d").longOpt("delay").hasArg(true).desc("delay in ms - by default: 200ms").build());
